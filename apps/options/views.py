...
def cost_center_options(request):
    qs = CostCenter.objects.select_related("legal_entity", "branch").filter(is_active=True).order_by("code")

    # Si tiene branch, filtramos por sucursal. Si no, por razón social.
    ...
    qs = scoped_by_branch | scoped_by_entity
    ...
    data = [
        ...
        for obj in qs.distinct().order_by("code")
    ]
...
def product_category_options(request):
    ...
    data = [
        serialize_option(obj, label=obj.name)
        for obj in qs
    ]
...
def warehouse_options(request):
    ...
    data = [
        serialize_option(
            obj,
            label=f"{obj.branch.name} - {obj.name}",
            extra={
